...
(X-Chickadee-Bridge-Secret) that the add-on provisions to its addon_config
(/config/addon_configs/chickadee/bridge_secret — readable by this integration, not by
...
#
#   family Console (`chickadee` / `chickadee_dev`)
#       header  X-Chickadee-Bridge-Secret
#       file    addon_configs/chickadee/bridge_secret
#       missing header → observe mode allows it
#
#   HA edition (`chickadee` / `chickadee_dev`, formerly Chickadee)
#       header  x-chickadee-voice-bridge-secret
#       file    <ha-config>/.chickadee/bridge_secret
...
_BRIDGE_HEADER = "X-Chickadee-Bridge-Secret"
_BRIDGE_HEADER_HA = "x-chickadee-voice-bridge-secret"
...
# (chickadee, chickadee_dev, chickadee, chickadee_dev). Enumerating them here would be a fourth
# copy of the slug vocabulary and would rot on the next rename — it already had: the list
# read only `addon_configs/chickadee/`, so the family Console's DEV channel (`chickadee_dev`) was
...
# loud one. `chickadee*` covers every present and future channel of both products without
# naming any of them, and is narrow enough not to read a sibling add-on's secret.
_BRIDGE_SECRET_GLOBS = (
    "addon_configs/chickadee*/bridge_secret",   # either product, any channel
    ".chickadee/bridge_secret",           # HA edition's HA-config-dir channel
)
...
# chickadee.refresh_voice_config on sign-in/sign-out (→ clear_credential_cache below), which
...
class AddonUnavailable(Exception):
    """The Chickadee add-on / account credential isn't reachable."""


class SharingDisabled(AddonUnavailable):
    """Add-on reachable + signed in, but household Chickadee Cloud sharing is off.
...
def _is_chickadee_addon(a: dict) -> bool:
    """Any Chickadee add-on, either PRODUCT and either channel.
    ...
    the field: the 07-30 Chickadee→Chickadee rename moved the HA add-on to `chickadee*`,
    which satisfies none of the old conditions — `_chickadee_dev` does not end with
    `_chickadee` or `chickadee_dev`, and "Chickadee" does not start with
    "Chickadee Console". Supervisor discovery therefore returned [], the caller fell back to
    the `local-chickadee` hostnames (valid only for a LOCAL install), and every add-on call
    ...
    return (
        slug == "chickadee"
        or slug.endswith("_chickadee")
        or slug.endswith("chickadee_dev")
        or slug == "chickadee"
        or slug.endswith("_chickadee")
        or slug.endswith("chickadee_dev")
        or name.startswith("Chickadee Console")
        or name.startswith("Chickadee")
    )
...
def _is_dev_addon(a: dict) -> bool:
    ...
    Matches on the `_dev` suffix rather than the old `chickadee_dev`: the HA-edition dev
    slug is `chickadee_dev`, which ends with `ha_dev`, so the narrower test silently
...
async def _discover_via_supervisor(session) -> list[str]:
    """Resolve Chickadee add-on base URLs (http://<ip>:8099) via the Supervisor API.

    Returns ALL installed Chickadee add-ons, **dev channel first** — a dev box runs
    ...
    try:
        ...
        matches = [a for a in addons if _is_chickadee_addon(a)]
        if not matches:
            _LOGGER.debug("chickadee add-on not found in supervisor list")
            return []
...
def clear_credential_cache() -> None:
    ...
    Called by the `chickadee.refresh_voice_config` service, which the add-on fires on sign-in and
...
async def authorize_device(hass: HomeAssistant, user_code: str) -> tuple[dict, int]:
    ...
    return ({"error": "addon_unavailable", "message": f"Chickadee add-on unreachable: {last_err}"}, 503)
...
async def mint_live_token(hass: HomeAssistant, model: str | None = None) -> tuple[dict, int]:
    ...
    conversation-relay as the x-chickadee-live-token header.
