...
Storage: homeassistant.helpers.storage.Store -> .storage/chickadee.hidden_speakers
HTTP endpoints:
  GET  /api/chickadee/music/hidden_speakers  — retrieve hidden speaker IDs
  POST /api/chickadee/music/hidden_speakers  — save hidden speaker IDs
"""
...
STORAGE_KEY = "chickadee.hidden_speakers"
STORAGE_VERSION = 1
...
class ChickadeeHiddenSpeakersView(HomeAssistantView):
    """Get or save the globally hidden speaker list."""

    url = "/api/chickadee/music/hidden_speakers"
    name = "api:chickadee:music:hidden_speakers"
    requires_auth = True

    async def get(self, request: web.Request) -> web.Response:
        hass = request.app["hass"]
        store: HiddenSpeakersStore | None = hass.data.get("chickadee", {}).get(
...
def register_hidden_speakers_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeHiddenSpeakersView())
