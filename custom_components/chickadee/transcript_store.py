...
Storage: homeassistant.helpers.storage.Store -> .storage/chickadee.voice_transcripts
...
HTTP endpoints (HA-token authed):
  GET    /api/chickadee/voice/transcripts        — recent transcripts (newest first)
  DELETE /api/chickadee/voice/transcripts        — clear all stored transcripts
"""
...
STORAGE_KEY = "chickadee.voice_transcripts"
STORAGE_VERSION = 1
MAX_TRANSCRIPTS = 500
...
class ChickadeeVoiceTranscriptsView(HomeAssistantView):
    """Read or clear HA-local voice transcripts."""

    url = "/api/chickadee/voice/transcripts"
    name = "api:chickadee:voice:transcripts"
    requires_auth = True

    async def get(self, request: web.Request) -> web.Response:
        hass: HomeAssistant = request.app["hass"]
        store: TranscriptStore | None = hass.data.get("chickadee", {}).get("transcript_store")
        if store is None:
...
def register_transcript_views(hass: HomeAssistant) -> None:
    """Register Chickadee voice transcript HTTP views."""
    hass.http.register_view(ChickadeeVoiceTranscriptsView())
