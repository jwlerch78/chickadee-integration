...
async def async_setup_entry(
    ...
    """Set up Chickadee media player from a config entry."""
    coordinator: ChickadeeCoordinator = hass.data[DOMAIN][entry.entry_id]
    device_id = entry.data[CONF_DEVICE_ID]
    async_add_entities([ChickadeeMediaPlayer(coordinator, device_id)])


class ChickadeeMediaPlayer(ChickadeeEntity, MediaPlayerEntity):
    """Representation of a Chickadee device as a media player.
    ...
    def __init__(
        self,
        coordinator: ChickadeeCoordinator,
        device_id: str,
    ) -> None:
