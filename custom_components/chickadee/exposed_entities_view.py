...
Voice Assistants → Expose). The Chickadee cloud/voice brain uses this to send only the
...
HTTP endpoint:
  GET /api/chickadee/exposed_entities
...
back-compat. Same shape is returned by /api/chickadee/dashboard_entities.
"""
...
class ChickadeeExposedEntitiesView(HomeAssistantView):
    """Return the entity IDs exposed to HA's conversation assistant."""

    url = "/api/chickadee/exposed_entities"
    name = "api:chickadee:exposed_entities"
    requires_auth = True
...
def _start_url_for_device(hass: HomeAssistant, device_id: str):
    """The startUrl (dashboard the device loads) for a Chickadee device, from its coordinator."""
...
class ChickadeeDashboardEntitiesView(HomeAssistantView):
    ...
    url = "/api/chickadee/dashboard_entities"
    name = "api:chickadee:dashboard_entities"
    requires_auth = True
...
def register_exposed_entities_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeExposedEntitiesView())
    hass.http.register_view(ChickadeeDashboardEntitiesView())
    _LOGGER.info("Registered Chickadee exposed-entities + dashboard-entities views")
