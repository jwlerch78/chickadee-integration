...
Storage: homeassistant.helpers.storage.Store -> .storage/chickadee.immich_token
HTTP endpoints:
  GET  /api/chickadee/immich/token  — retrieve stored token + server URL + albums
  POST /api/chickadee/immich/token  — save token + server URL + albums (from first device to login)
  DELETE /api/chickadee/immich/token — clear stored credentials (sign out)
"""
...
STORAGE_KEY = "chickadee.immich_token"
STORAGE_VERSION = 1
...
class ChickadeeImmichTokenView(HomeAssistantView):
    """Get or save the central Immich token."""

    url = "/api/chickadee/immich/token"
    name = "api:chickadee:immich:token"
    requires_auth = True

    async def get(self, request: web.Request) -> web.Response:
        hass = request.app["hass"]
        store: ImmichTokenStore | None = hass.data.get("chickadee", {}).get(
            "immich_token_store"
        )
        if store is None:
...
def register_immich_token_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeImmichTokenView())
