...
Storage: homeassistant.helpers.storage.Store -> .storage/chickadee.video_feeds
...
STORAGE_KEY = "chickadee.video_feeds"
STORAGE_VERSION = 1
...
class ChickadeeFeedsListView(HomeAssistantView):
    """List all feeds or create/update a feed."""

    url = "/api/chickadee/feeds"
    name = "api:chickadee:feeds"
    requires_auth = True
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        registry: FeedRegistry = hass.data["chickadee"]["feed_registry"]
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        registry: FeedRegistry = request.app["hass"].data["chickadee"]["feed_registry"]
        try:
...
class ChickadeeFeedDeleteView(HomeAssistantView):
    """Delete a feed."""

    url = "/api/chickadee/feeds/{feed_id}"
    name = "api:chickadee:feeds:delete"
    requires_auth = True
...
class ChickadeeSubscriptionView(HomeAssistantView):
    """Get or update a device's feed subscriptions."""

    url = "/api/chickadee/feeds/subscriptions/{device_id}"
    name = "api:chickadee:feeds:subscriptions"
    requires_auth = True
...
async def get_frigate_camera_for_entity(
    ...
    """
    registry: FeedRegistry | None = hass.data.get("chickadee", {}).get("feed_registry")
    ...
    for feed in registry._data["feeds"].values():
        ...
        # honour overrides set since the last ChickadeeFeedsListView GET.
        _annotate_frigate_camera(feed, frigate_cameras)
...
def register_feed_registry_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeFeedsListView())
    hass.http.register_view(ChickadeeFeedDeleteView())
    hass.http.register_view(ChickadeeSubscriptionView())
