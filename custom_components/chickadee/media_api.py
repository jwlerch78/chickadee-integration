...
1. CHICKADEE_MEDIA_PATH environment variable
...
def _get_media_base_path(hass: HomeAssistant) -> Path:
    ...
    env_path = os.environ.get("CHICKADEE_MEDIA_PATH")
    if env_path:
        _LOGGER.info("Using CHICKADEE_MEDIA_PATH: %s", env_path)
...
class ChickadeeMediaListView(HomeAssistantView):
    """View to list photos from HA's media folder."""

    url = "/api/chickadee/media"
    name = "api:chickadee:media"
    requires_auth = True
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        Query params:
        - folder: Subfolder within /config/media (default: "chickadee-photos")
...
class ChickadeeMediaImageView(HomeAssistantView):
    """View to serve a photo from HA's media folder."""

    url = "/api/chickadee/media/image/{folder}/{filename:.*}"
    name = "api:chickadee:media:image"
    requires_auth = True
...
class ChickadeeMediaFoldersView(HomeAssistantView):
    """View to list available folders in HA's media directory."""

    url = "/api/chickadee/media/folders"
    name = "api:chickadee:media:folders"
    requires_auth = True
                ...
                if item.is_file():
                    ...
                    if ext in SUPPORTED_EXTENSIONS:
                        ...
                        photos.append({
                            "filename": item.name,
                            "path": relative_path,
                            "url": f"/api/chickadee/media/image/{folder_name}/{relative_path}",
        ...
        if item.is_file():
            ...
            if ext in SUPPORTED_EXTENSIONS:
                photos.append({
                    "filename": item.name,
                    "path": item.name,
                    "url": f"/api/chickadee/media/image/./{item.name}",
                    "size": item.stat().st_size,
                    "modified": item.stat().st_mtime,
                })
        elif item.is_dir() and not item.name.startswith("."):
...
def register_media_api_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeMediaListView())
    hass.http.register_view(ChickadeeMediaImageView())
    hass.http.register_view(ChickadeeMediaFoldersView())
