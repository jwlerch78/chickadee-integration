...
Endpoint: GET /api/chickadee/stream/feed/{feed_id}
"""
...
class ChickadeeFeedStreamView(HomeAssistantView):
    """Serve shared MJPEG stream for a feed ID."""

    url = "/api/chickadee/stream/feed/{feed_id}"
    name = "api:chickadee:stream:feed"
    requires_auth = True
...
def register_stream_multiplexer_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeFeedStreamView())
    _LOGGER.info("Registered Chickadee feed stream multiplexer view")
