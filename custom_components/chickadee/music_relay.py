...
HTTP endpoints:
  POST /api/chickadee/music/relay       — relay an MA API command (REST)
  POST /api/chickadee/music/ws-command  — relay a single command via MA WebSocket
  GET  /api/chickadee/music/imageproxy  — relay an MA image proxy request
"""
...
class ChickadeeMusicRelayView(HomeAssistantView):
    ...
    url = "/api/chickadee/music/relay"
    name = "api:chickadee:music:relay"
    requires_auth = True

    async def post(self, request: web.Request) -> web.Response:
        hass: HomeAssistant = request.app["hass"]
        store: MusicTokenStore | None = hass.data.get("chickadee", {}).get(
            "music_token_store"
        )
        if store is None:
...
class ChickadeeMusicImageProxyView(HomeAssistantView):
    ...
    url = "/api/chickadee/music/imageproxy"
    name = "api:chickadee:music:imageproxy"
    requires_auth = True
...
class ChickadeeMusicWsCommandView(HomeAssistantView):
    ...
    url = "/api/chickadee/music/ws-command"
    name = "api:chickadee:music:ws_command"
    requires_auth = True
...
def register_music_relay_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeMusicRelayView())
    hass.http.register_view(ChickadeeMusicImageProxyView())
    hass.http.register_view(ChickadeeMusicWsCommandView())
