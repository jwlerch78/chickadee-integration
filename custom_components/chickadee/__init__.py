...
"""Chickadee integration for Home Assistant."""
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the Chickadee integration."""
    hass.data.setdefault(DOMAIN, {})
    return True


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Chickadee from a config entry."""
    ...
    coordinator = ChickadeeCoordinator(hass, host, port, password, config_entry=entry)
    ...
    if not coordinator.last_update_success and hass.state is not CoreState.running:
        ...
        if not entities:
            _LOGGER.warning(
                "Removing orphaned Chickadee config entry for %s (%s) — "
    ...
    if "feed_registry" not in hass.data[DOMAIN]:
        ...
        _LOGGER.info("Initialized Chickadee feed registry")
    ...
    if not _feed_registry_registered:
        ...
        _LOGGER.info("Registered Chickadee feed registry views")
    ...
    if "music_token_store" not in hass.data[DOMAIN]:
        ...
        _LOGGER.info("Initialized Chickadee music token store")
    ...
    if "immich_token_store" not in hass.data[DOMAIN]:
        ...
        _LOGGER.info("Initialized Chickadee Immich token store")

    if not _immich_token_registered:
        register_immich_token_views(hass)
        _immich_token_registered = True
        _LOGGER.info("Registered Chickadee Immich token views")
    ...
    if "hidden_speakers_store" not in hass.data[DOMAIN]:
        ...
        _LOGGER.info("Initialized Chickadee hidden speakers store")
    ...
    if not getattr(register_hidden_speakers_views, '_registered', False):
        ...
        _LOGGER.info("Registered Chickadee hidden speakers views")

    if not _music_relay_registered:
        register_music_relay_views(hass)
        _music_relay_registered = True
        _LOGGER.info("Registered Chickadee music relay views")

    if not _multiplexer_registered:
        register_stream_multiplexer_views(hass)
        _multiplexer_registered = True
        _LOGGER.info("Registered Chickadee stream multiplexer views")

    if not _sensor_push_registered:
        register_sensor_push_views(hass)
        _sensor_push_registered = True
        _LOGGER.info("Registered Chickadee sensor push endpoint")

    # This integration ships no voice views: the voice integration owns the
    # gateway here (CONTRACTS #60). Cede UNCONDITIONALLY — the sibling only
    # registers when it sees this flag, so leaving it unset would mean nobody
    # serves voice, with both integrations healthy and no error either side.
    ...
    if "transcript_store" not in hass.data[DOMAIN]:
        ...
        _LOGGER.info("Initialized Chickadee voice transcript store")
    ...
    if not getattr(register_transcript_views, '_registered', False):
        ...
        _LOGGER.info("Registered Chickadee voice transcript views")
    ...
    if not getattr(register_exposed_entities_views, '_registered', False):
        ...
        _LOGGER.info("Registered Chickadee exposed-entities view")

    if not _frigate_proxy_registered:
        register_frigate_proxy_views(hass)
        _frigate_proxy_registered = True
        _LOGGER.info("Registered Chickadee Frigate proxy views")
...
async def _async_migrate_device_id_if_needed(
    hass: HomeAssistant, entry: ConfigEntry, coordinator: ChickadeeCoordinator
) -> None:
...
async def _async_register_services(hass: HomeAssistant) -> None:
    """Register Chickadee services."""

    def _get_all_coordinators() -> list[ChickadeeCoordinator]:
        """Get all coordinators (filter out non-coordinator items)."""
        return [
            v for v in hass.data[DOMAIN].values()
            if isinstance(v, ChickadeeCoordinator)
        ]
    ...
    async def async_refresh_voice_config(call: ServiceCall) -> None:
        """Push a voice-config refresh to Chickadee devices (anon-kiosk mirror).

        Fired by the add-on when household Chickadee Cloud sharing is toggled (or the
        account's voice settings change). Each device re-probes
        /api/chickadee/voice/status and hard-applies the account's voice pipeline — so a
        ...
        # "refreshVoiceConfig" matches the ChickadeeApiServer cmd on the device's 2323 API.
        for coordinator in _get_all_coordinators():
    ...
    async def _send_timer_to_devices(timer: dict, action: str = "update") -> None:
        """Send timer state to all Chickadee devices."""
    ...
    async def _hide_timer_from_devices(timer_id: str, slot: int) -> None:
        """Hide timer from all Chickadee devices."""
        for coordinator in _get_all_coordinators():
            await coordinator.send_command(
                "hideTimer",
                timerId=timer_id,
    ...
    _LOGGER.info("Registered Chickadee services")
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator: ChickadeeCoordinator | None = hass.data.get(DOMAIN, {}).get(entry.entry_id)
    if coordinator:
...
async def async_remove_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    _LOGGER.info("Cleaned up removed Chickadee device %s (%s)", device_id, host)
