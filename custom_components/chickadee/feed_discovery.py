...
page) render this list and POST a chosen camera back to ``/api/chickadee/feeds``.
...
(no stream source — e.g. a Chickadee tablet camera that's turned off) is hidden
...
class ChickadeeFeedsDiscoverView(HomeAssistantView):
    """GET /api/chickadee/feeds/discover — cameras the user could add as feeds."""

    url = "/api/chickadee/feeds/discover"
    name = "api:chickadee:feeds:discover"
    requires_auth = True

    async def get(self, request: web.Request) -> web.Response:
        hass = request.app["hass"]
        registry = hass.data.get("chickadee", {}).get("feed_registry")
        ...
        for entity_id, friendly, frigate, source in evaluated:
            ...
            by_key[key] = {
                ...
                "snapshot_url": f"/api/chickadee/stream/snapshot/{entity_id}",
            }
...
def register_feed_discovery_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeFeedsDiscoverView())
    _LOGGER.info("Registered Chickadee feed discovery view")
