...
"""DataUpdateCoordinator for Chickadee."""
...
class ChickadeeCoordinator(DataUpdateCoordinator):
    """Coordinator to manage fetching data from Chickadee device."""
    ...
    def _reset_backoff(self) -> None:
        ...
        if self._consecutive_failures > 0:
            ...
            _LOGGER.info(
                "Reconnected to Chickadee device at %s after %d failures (caller: %s)",
    ...
    async def _async_update_data(self) -> dict:
        """Fetch data from Chickadee device."""
        try:
    ...
    async def send_command(self, command: str, **kwargs) -> bool:
        """Send a command to the Chickadee device."""
        try:
            session = await self._get_session()
            params = {"cmd": command}
            if self.password:
