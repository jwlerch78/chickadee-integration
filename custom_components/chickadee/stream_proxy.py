...
Endpoint: GET /api/chickadee/stream/mjpeg/{entity_id}?fps=10&quality=8&width=640
...
class ChickadeeMjpegStreamView(HomeAssistantView):
    """Serve real-time MJPEG stream for any HA camera entity."""

    url = "/api/chickadee/stream/mjpeg/{entity_id:.*}"
    name = "api:chickadee:stream:mjpeg"
...
class ChickadeeSnapshotView(HomeAssistantView):
    ...
    url = "/api/chickadee/stream/snapshot/{entity_id:.*}"
    name = "api:chickadee:stream:snapshot"
    requires_auth = False  # Matches ChickadeeMjpegStreamView for browser testing
...
def register_stream_proxy_views(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(ChickadeeMjpegStreamView())
    hass.http.register_view(ChickadeeSnapshotView())
    _LOGGER.info("Registered Chickadee MJPEG stream proxy + snapshot views")
