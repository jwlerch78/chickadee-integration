...
async def async_setup_entry(
    ...
    """Set up Chickadee update entity."""
    ...
    update_coordinator = ChickadeeUpdateCoordinator(hass)

    # Do initial fetch
    await update_coordinator.async_config_entry_first_refresh()

    async_add_entities([ChickadeeUpdateEntity(update_coordinator, entry, current_version)])


class ChickadeeUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Coordinator to check for Chickadee integration updates."""
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        super().__init__(
            hass,
            _LOGGER,
            name="Chickadee Integration Update",
...
class ChickadeeUpdateEntity(CoordinatorEntity[ChickadeeUpdateCoordinator], UpdateEntity):
    """Update entity for Chickadee integration.
    ...
    without creating a separate "Chickadee Integration" device.
    """

    _attr_has_entity_name = False  # Use name directly, not device + entity name
    _attr_device_class = UpdateDeviceClass.FIRMWARE
    _attr_supported_features = UpdateEntityFeature.RELEASE_NOTES
    _attr_title = "Chickadee Integration"

    def __init__(
        self,
        coordinator: ChickadeeUpdateCoordinator,
        entry: ConfigEntry,
        current_version: str,
    ) -> None:
        ...
        self._attr_name = "Chickadee Integration Update"
        self._current_version = current_version

    # Note: No device_info property - this entity is standalone and doesn't
    # create a separate device. This prevents the "Chickadee Integration" device
    # from appearing under each Chickadee device in the UI.
